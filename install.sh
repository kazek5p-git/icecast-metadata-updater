...
print_help() {
  cat <<EOF
Uzycie:
  ./install.sh [--install-dir KATALOG]

Przyklad:
  ./install.sh --install-dir "$HOME/icecast-metadata-updater"
EOF
}
...
chmod +x "$INSTALL_DIR/start_updater.sh"
...
if [[ ! -f "$INSTALL_DIR/config.json" ]]; then
  ...
  echo "Utworzono $INSTALL_DIR/config.json"
else
