...
Opcje:
  ...
  --public-key PLIK       Publiczny klucz PEM do weryfikacji podpisu paczki
  --allow-unsigned        Dopuszczaj manifest bez podpisu (mniej bezpieczne)
  -h, --help              Pomoc
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --yes)
      ...
      ;;
    --public-key)
      [[ $# -lt 2 ]] && { echo "Brak wartosci dla --public-key" >&2; exit 1; }
      SIGNING_PUBLIC_KEY_PATH="$2"
      shift 2
      ;;
    --allow-unsigned)
      ALLOW_UNSIGNED=1
...
if [[ -n "$MANIFEST_SIGNATURE" ]]; then
  ...
  require_cmd openssl
  SIGNATURE_PATH="$TMP_DIR/package.sig"
...
PY
  ...
  if [[ "$ALLOW_UNSIGNED" -eq 1 ]]; then
    echo "Uwaga: aktywny --allow-unsigned, pomijam kontrole podpisu."
  else
    echo "Uwaga: manifest bez podpisu cyfrowego. Zalecane podpisane wydania."
  fi
fi
